...
`requiredSecrets` contains aliases only; values are resolved at run time. The
optional `taskId` links the test to a repository task. Put optional `platforms`,
`deviceKinds`, and `frameworks` metadata on that task descriptor; the CLI runner
validates it against the selected target and connected session.
