...
A small .NET MAUI app for checking whether synthetic speech reaches an iOS Simulator or Android Emulator through its microphone. Both platforms can validate a final transcript or save microphone samples for direct waveform verification. Application ID: `ai.ansight.audioharness`.

```text
Host WAV playback/injector → virtual device microphone → native speech recognizer → final transcript assertion
                                                     → microphone WAV → whole-waveform assertion
```

The host injects the speech track. The app receives ordinary microphone audio and uses Apple Speech or Android's installed speech provider. The expected phrase is used only for the final assertion; it is never supplied to recognition as audio, a prompt, or a recognition hint. There is no in-app file-to-transcript shortcut.
...
This retains the Debug configuration and Ansight SDK but omits Android debugging support, so `adb run-as` is unavailable. Use **Copy result** for result export from that package.

## Permissions

The app requests microphone access on both platforms and speech-recognition access on iOS when transcription is selected. Capture-only mode requests microphone access only and uses native AVAudioEngine on iOS or AudioRecord on Android. Android runs AudioRecord and SpeechRecognizer in separate modes. Both iOS usage descriptions and Android's `RECORD_AUDIO` permission are declared in the platform manifests. A denied permission produces a failed run and cannot pass validation.
...
**Android Emulator:** the verified route uses the emulator's `injectAudio` gRPC API with host microphone access disabled. No virtual audio driver is needed for this route. The legacy runner uses the sibling checkout's `../../ansight/scripts/audio-input/inject_audio.py` and the host Python environment prepared above. The harness must already be open, have microphone permission, and have a working Android speech provider.
...
1. Open the app, check the expected phrase, and tap **Start listening**. Accept requested permissions.
2. Wait until `capture-phase` is **Listening** before starting external fixture playback or injection.
3. Allow the provider to finish after silence, or tap **Stop** after the entire padded clip. The app also limits listening to approximately 30 seconds and waits for finalization.
...
Apple Speech and the Android provider must be available for English (US); network access or downloaded language models may be required. Android emulator images without a speech provider show an error; use an image with a working provider. The iOS level meter measures microphone samples, while Android reports the provider's activity estimate. Neither meter alone proves a recognized phrase. A successful build or launch also does not prove end-to-end injected-audio transcription; validate it using the final assertions above.
